...
export async function GET(req) {
  ...
  try {
    ...
    // Agar ids diye gaye → unhi products fetch karo
    if (idsParam) {
    ...
    const products = await Product.find(query)
      .populate("category")
      .sort({ createdAt: -1 })
      .skip(idsParam ? 0 : (page - 1) * limit) // agar ids diye to pagination skip na ho
      .limit(idsParam ? idsParam.split(",").length : limit);

    return responseHelper.success(
      {
        data: products,
        page: idsParam ? 1 : page,
        totalPages: idsParam ? 1 : Math.ceil(total / limit),
        total,
      },
      "Products fetched"
    );
