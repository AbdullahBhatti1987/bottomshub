...
export default function Reviews() {
  const [reviews, setReviews] = useState([]);
  const [current, setCurrent] = useState(0);
  const [perSlide, setPerSlide] = useState(2); // default desktop
  ...
  // Detect screen width to set reviews per slide
  useEffect(() => {
    const handleResize = () => {
      setPerSlide(window.innerWidth < 640 ? 1 : 2); // mobile sm < 640px = 1
    };
    handleResize();
  ...
  // Auto-slide every 4s
  useEffect(() => {
    if (reviews.length <= perSlide) return;
    const timer = setInterval(() => {
      setCurrent((prev) => (prev + perSlide) % reviews.length);
    }, 4000);
    return () => clearInterval(timer);
  }, [reviews.length, perSlide]);
      ...
      <div className="max-w-5xl mx-auto text-center px-6">
        ...
        <div className="relative flex justify-center">
          <AnimatePresence mode="wait">
            <motion.div
              key={current}
              className={`grid grid-cols-1 sm:grid-cols-${perSlide} gap-6`}
        ...
        {/* Slider dots */}
        <div className="flex justify-center mt-6 gap-2">
          {Array.from({ length: Math.ceil(reviews.length / perSlide) }).map(
            (_, idx) => (
              <button
                key={idx}
                onClick={() => setCurrent(idx * perSlide)}
                className={`w-3 h-3 rounded-full transition ${
                  idx * perSlide === current ? "bg-gray-800" : "bg-gray-400"
                }`}
              />
            )
          )}
