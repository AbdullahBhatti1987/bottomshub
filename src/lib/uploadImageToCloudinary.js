...
export const uploadImageToCloudinary = async (fileBase64, folder = 'bottomshub/products') => {
  try {
    // Upload the image with max width 1024, keeping aspect ratio
    const uploadResponse = await cloudinary.uploader.upload(fileBase64, {
      folder,
      transformation: [
    ...
    const { secure_url, public_id, width, height } = uploadResponse;
    ...
    return {
      url: secure_url,     // Original image with width <= 1024, aspect ratio preserved
      thumbnailUrl,        // Thumbnail with max width 100, ratio preserved
      public_id,
      width,
      height
    };
